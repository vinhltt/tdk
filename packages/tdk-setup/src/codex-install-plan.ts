...
function readArtifacts(input: BuildCodexInstallPlanInput): { artifacts: CodexInstallArtifact[]; configBlocks: string[]; hasHooks: boolean; } {
  ...
  for (const plugin of input.selectedPlugins) {
    ...
    if (!sourceEntry) throw new Error(`Unknown plugin "${plugin}" in .specify/plugins/manifest.json.`);
    const sourceFiles = sourceEntry.files ?? {};
    const agentFiles = Object.keys(sourceFiles)
    ...
    for (const agentRelativePath of agentFiles) {
      ...
      if (!fs.existsSync(agentSourcePath)) throw new Error(`Source agent is missing for plugin "${plugin}": ${agentRelativePath}`);
      const stat = fs.lstatSync(agentSourcePath);
      if (stat.isSymbolicLink() || !stat.isFile()) throw new Error(`Refusing non-file source agent: ${plugin}/${agentRelativePath}`);
      const agentMd = fs.readFileSync(agentSourcePath, 'utf-8');
      const expectedAgentChecksum = sourceFiles[agentRelativePath]!;
      const actualAgentChecksum = sha256Buffer(Buffer.from(agentMd, 'utf-8'));
      if (actualAgentChecksum !== expectedAgentChecksum) throw new Error(`Source agent checksum mismatch: ${plugin}/${agentRelativePath}`);
      ...
      artifacts.push({
        ...
        sourceChecksum: expectedAgentChecksum,
