...
The script checks prerequisites, installs Bun when missing, installs TypeScript setup dependencies, creates or verifies the Python venv, checks config detection, and registers available plugin marketplaces.
...
- Enable the Context7 plugin when docs-seeker support is needed.
...
| Plugin registration failed | Install Claude Code, then re-run setup without `--skip-plugins`. |
