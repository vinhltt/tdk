...
description: "Project-level workspace boundary proposal workflow. Writes topology proposal markdown and JSON without mutating runtime config."
user-invocable: true
...
Map project architecture evidence into reviewable workspace topology proposal
...
This command is **topology proposal only**.
...
create directories, does not move or rename source folders, does not scaffold
source, does not enforce module boundaries, does not write tracker issues, and
does not write ADR files.**

Boundary summary: does not create directories; does not move or rename source
...
- keep topology proposal separate from runtime config;
...
Use the existing proposal directory:

```text
.specify/configurations/workspace-topology/
```

If the directory is missing, stop and report that path as required setup. Do not
create directories in this slice.
