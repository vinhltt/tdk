...
### Sequential Thinking (Question & Category Prioritization)

**Trigger:** During Step 2 (clarifying questions) and Step 5 (checklist generation).

**For question generation (Step 2):**
- After extracting signals and clustering into focus areas
- Rank focus areas by: risk level > coverage gap size > user emphasis
- Generate questions targeting highest-ranked areas first

**For checklist generation (Step 5):**
- After identifying all requirement quality dimensions
- Order categories by: most findings > highest severity gaps > user-specified focus
- Ensure coverage balance: don't over-index on one category

## Execution Steps
### Step 0 — Validate Task ID
Invoke `tdk-validate-task-id` with `$ARGUMENTS` and host skill name `/tdk-checklist`.
...
2. **Clarify intent (dynamic)**: Derive up to THREE initial contextual clarifying questions (no pre-baked catalog). They MUST:
   ...
   2b. Apply sequential thinking to rank clusters:
       - Score each cluster: (risk indicator count) x (coverage gap severity)
       - Order by score descending
       - Ensure at least one cluster addresses user's explicit focus
...
4. **Load feature context**: Read from FEATURE_DIR:
   ...
   - plan.md (if exists): Technical details, dependencies, and `## Phases` table
...
5. **Generate checklist** - Create "Unit Tests for Requirements":
   ...
   - Each `/tdk-checklist` run creates a NEW file (never overwrites existing checklists)
   ...
   **Sequential Thinking for Category Ordering:**
   - List all applicable quality dimensions for this checklist
   - Order by: dimensions with most findings first, then by severity
   - Ensure no dimension with critical findings is buried at the end
...
**Important**: Each `/tdk-checklist` command invocation creates a checklist file using short, descriptive names unless file already exists. This allows:
