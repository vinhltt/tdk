...
**`--fast` interaction:** tdk-memory-agent `--mode validate` still runs (Key Constraint #2 + S2.F7). Negligible token cost (~500) vs. regression risk from bypassing binding invariants. See `references/modes.md`. When MCP_UNAVAILABLE in --fast: auto re-spawn with --no-mcp + warning log (no AskUserQuestion, to preserve speed).
