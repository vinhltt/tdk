...
Origin plan: [`plans/260510-1624-tdk-sub-workspace-docs/`](../../../../../plans/260510-1624-tdk-sub-workspace-docs).
...
- `tdk-scout` skill installed (this skill spawns it per-target). Check: `ls .specify/plugins/tdk-utils/skills/tdk-scout/`.
...
2. **Pre-check tdk-scout skill is installed.**
   ```bash
   ls .specify/plugins/tdk-utils/skills/tdk-scout/SKILL.md
   ```
   If missing: tell the user `tdk-scout` skill is required and stop. (Decision chốt: fail hard.)
...
4. **Parse contract.** Take the LAST stdout line and `JSON.parse` it. On `{ ok: false, error, code }`: surface the error to the user with code, then stop.
...
5. **Per-target sequential loop.** For each `target` in `targets`:

   ### 5a. (mode=update only) Optional user feedback
   Use `AskUserQuestion`:
   ...
   ### 5b. Legacy marker check (mode=update only)
   ...
   ### 5c. Spawn tdk-scout
   Use the Task tool:
   - description: `"Scout sub-workspace <target.name>"`
   - subagent_type: `tdk-scout-runner` is for the runner; here we invoke the **skill** by running its CLI flow. Easiest: `Bash`:
     ```bash
   ...
   - prompt (JSON-encoded contract):
     ```json
     {
       ...
       "userFeedback": "<5a result or empty>",
...
6. **Cleanup prompt** (after all targets processed):
...
7. **Final summary.** Print one block per target:
   ```
   <target.name> [<mode>]
     written: <n> file(s)
     warnings: <n>
...
- This skill replaces the codebase-analysis half of claudekit `/ck:docs init`. PRD/roadmap orchestration is out of scope.
