# From Existing Workspace Layout Proposal Workflow
...
4. Put observed real folders or packages only in `workspace-layout-proposal.json` by
   default.
5. Keep desired-state deltas stay in `workspace-layout-proposal.md` until the user
   explicitly accepts them as current layout.
6. Record ownership, dependency direction, and next route as report-only fields.
7. Write `workspace-layout-proposal.md` and `workspace-layout-proposal.json` when observed
...
or layout would depend only on weak naming inference.
