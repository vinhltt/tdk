# Unknown Workspace Layout Proposal Workflow
...
3. List missing evidence needed for a workspace layout proposal.
4. Record assumptions only when explicitly accepted or clearly marked weak.
5. Do not make a strong workspace layout proposal from weak evidence.
6. Write `workspace-layout-proposal.md` as readiness guidance when useful.
7. Do not overwrite `workspace-layout-proposal.json` when evidence is insufficient.
...
- Recommend `/tdk-scout` when repo structure, package ownership, or file roles
  are unclear.
- Recommend standard workspace-layout-propose mode only when architecture and path evidence
  are sufficient.
